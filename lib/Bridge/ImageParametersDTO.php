...
class ImageParametersDTO
{
            ...
            switch ($fid) {
                case 1:
                    if ($ftype == TType::STRUCT) {
                        $this->svg = new \Aspose\Barcode\Bridge\SvgParametersDTO();
                        $xfer += $this->svg->read($input);
                    } else {
    ...
    {
        $xfer = 0;
        $xfer += $output->writeStructBegin('ImageParametersDTO');
        if ($this->svg !== null) {
            if (!is_object($this->svg)) {
